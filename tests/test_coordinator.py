...
class TestWEZCircuitType:
    """Tests that WEZ (heat generator) circuit type is correctly handled.

    WEZ is the Hoval v3 API type for heat-generator / boiler circuits.
    It shares live-value keys with BL and must be treated as non-selectable
    (selectable=False is expected from the API; we still fetch its live values).
    """

    def _make_wez_circuit_data(self, live_values: dict | None = None) -> HovalCircuitData:
        return HovalCircuitData(
            circuit_type="WEZ",
            path="1.1.1.1.4.1",
            name="Heat Generator",
    ...
    def test_wez_circuit_data_instantiates(self):
        circuit = self._make_wez_circuit_data()
        assert circuit.circuit_type == "WEZ"
        assert circuit.path == "1.1.1.1.4.1"

    def test_wez_live_values_temp_actual(self):
        circuit = self._make_wez_circuit_data({"tempActual": "58.3"})
        assert circuit.live_values.get("tempActual") == "58.3"

    def test_wez_live_values_temp_target(self):
        circuit = self._make_wez_circuit_data({"tempTarget": "60.0"})
        assert circuit.live_values.get("tempTarget") == "60.0"

    def test_wez_live_values_return_temperature(self):
        circuit = self._make_wez_circuit_data({"returnTemperature": "42.1"})
        assert circuit.live_values.get("returnTemperature") == "42.1"

    def test_wez_live_values_operating_hours(self):
        circuit = self._make_wez_circuit_data({"operatingHours": "4321"})
        assert circuit.live_values.get("operatingHours") == "4321"

    def test_wez_live_values_operation_cycles(self):
        circuit = self._make_wez_circuit_data({"operationCycles": "812"})
        assert circuit.live_values.get("operationCycles") == "812"

    def test_wez_live_values_heat_amount(self):
        circuit = self._make_wez_circuit_data({"heatAmount": "12.34"})
        assert circuit.live_values.get("heatAmount") == "12.34"

    def test_wez_live_values_total_energy(self):
        circuit = self._make_wez_circuit_data({"totalEnergy": "5.67"})
        assert circuit.live_values.get("totalEnergy") == "5.67"

    def test_wez_missing_key_returns_none(self):
        """If a WEZ circuit doesn't provide a key, live_values.get returns None — sensors go unavailable."""
        circuit = self._make_wez_circuit_data({})
        assert circuit.live_values.get("tempActual") is None
        assert circuit.live_values.get("operatingHours") is None

    def test_wez_circuit_status(self):
        circuit = self._make_wez_circuit_data()
        circuit.circuit_status = "HEATING"
        assert circuit.circuit_status == "HEATING"

    def test_wez_is_in_supported_circuit_types(self):
        from custom_components.hoval_connect.const import SUPPORTED_CIRCUIT_TYPES, CIRCUIT_TYPE_WEZ
        assert CIRCUIT_TYPE_WEZ in SUPPORTED_CIRCUIT_TYPES

    def test_wez_circuit_type_name_defined(self):
        from custom_components.hoval_connect.const import CIRCUIT_TYPE_NAMES, CIRCUIT_TYPE_WEZ
        assert CIRCUIT_TYPE_WEZ in CIRCUIT_TYPE_NAMES
        assert CIRCUIT_TYPE_NAMES[CIRCUIT_TYPE_WEZ] == "Heat Generator"

    def test_resolve_fan_speed_ignores_wez(self):
        """resolve_fan_speed is HV-only; WEZ circuits should fall through to the default."""
        circuit = self._make_wez_circuit_data({"tempActual": "55.0"})
        # WEZ has no airVolume; neither target_value nor program_air_volume set
        assert resolve_fan_speed(circuit) == 40
